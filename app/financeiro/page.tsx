...
import { TrendingUp, TrendingDown, Wallet, Settings, PlusCircle, ArrowUpRight, ArrowDownLeft, Trash2, Loader2 } from 'lucide-react';
...
type Conta = { id: string; nome: string; saldo: string; tipo: string; cor: string; user_id?: string; created_at?: string; };
...
export default function Financeiro() {
  ...
  const handleAddTransacao = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title.trim() || !formData.value || !userId) return;
    ...
    const { data } = await supabase.from('transacoes').insert([newTransacao]).select().single();
    if (data) setTransacoes([data as Transacao, ...transacoes]);
  ...
  const handleSaveConta = async () => {
    if (!contaForm.nome.trim() || !userId) return;
    setIsSaving(true);

    const contaData = {
      nome: contaForm.nome,
      saldo: contaForm.saldo,
      tipo: contaForm.tipo,
      cor: contaForm.cor,
      user_id: userId
    };
    ...
    if (editingContaId) {
      const { data } = await supabase.from('contas').update(contaData).eq('id', editingContaId).select().single();
      if (data) setContas(contas.map((c) => c.id === editingContaId ? data as Conta : c));
    } else {
      const { data } = await supabase.from('contas').insert([contaData]).select().single();
      if (data) setContas([data as Conta, ...contas]);
    }
  ...
  const handleDeleteConta = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleEditConta = useCallback((conta: Conta, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    <div className="flex flex-col min-h-screen">
        ...
        <form onSubmit={handleAddTransacao} className="space-y-4">
          ...
          <div className="pt-4">
            <button disabled={isSaving} type="submit" className="w-full bg-primary hover:bg-primary/90 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-colors">
              {isSaving ? <Loader2 className="animate-spin text-white flex mx-auto" size={20} /> : 'Salvar Transação'}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={isAccountModalOpen} onClose={() => setIsAccountModalOpen(false)} title={editingContaId ? 'Editar Conta' : 'Nova Conta'}>
        <div className="flex flex-col gap-4">
          ...
          <button disabled={isSaving} onClick={handleSaveConta} className="w-full bg-primary hover:bg-primary/90 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-colors mt-2">
            {isSaving ? <Loader2 className="animate-spin text-white flex mx-auto" size={20} /> : (editingContaId ? 'Salvar Alterações' : 'Adicionar Conta')}
          </button>
        </div>
      </Modal>
    </div>
